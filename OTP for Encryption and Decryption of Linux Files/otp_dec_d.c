...
void decrypt(char transmission[], char key[]);
...
int main(int argc, char *argv[]) {
	...
	while(1) 
	{
		...
		if (processID == 0)
		{
			...
			//encode
			decrypt(transmission, keyPointer);
...
void decrypt(char transmission[], char key[]){
	...
	{
		//convert transmissionand key to int and subtract key from transmission
		c = characterToInteger(transmission[i]) - characterToInteger(key[i]);
		//if it is less than 0 add 27
		...
		transmission[i] = integerToCharacter(c);
	}
